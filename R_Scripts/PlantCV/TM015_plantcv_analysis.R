...
mean.ratio.area.100to30 <- trait.heatmap(traits.avg, "100", "30", "Ratio of 100 to 30", "area", "ratio",
  save = TRUE, save.name = "mean.ratio.area.100to30", big = TRUE)
...
mean.ratio.WUE.100to30 <- trait.heatmap(wue.data.agg, "100", "30", "Ratio of 100 to 30", "WUE", "ratio",
  save = TRUE, save.name = "mean.ratio.WUE.100to30", day.col = "day", big = TRUE)
...
set.seed(1320)
## a <- kmeans(traits.avg[, names.numeric.cols], 4, nstart = 100)
## a$cluster <- as.factor(a$cluster)
## traits.avg$cluster <- paste(a$cluster, traits.avg.day$treatment, sep = "-")

## traits.avg.bak <- traits.avg
## traits.avg <- traits.avg.bak[traits.avg.bak[, "treatment"] == 30, ]
## traits.avg
...
for( day in sort(unique(traits.avg$day)) ) {
  ...
  a <- kmeans(traits.avg.day[, numeric.traits.avg], 4, nstart = 100)
  a$cluster <- as.factor(a$cluster)
  traits.avg.day$cluster <- paste(a$cluster, traits.avg.day$treatment, sep = "-")
  ...
  p <- fviz_pca_biplot(shapes.pca, label="all", alpha.var = "contrib",
    ...
    habillage= traits.avg.day[, "cluster"],addEllipses = T, invisible = "ind") +
    ## xlim(c(-8,12)) +
    ## ylim(c(-5,5)) +
...
##### Running one to many comparisons
b73.vs.all <- oneToManyComps(traits, genotypes, "area", "B73")


## Taking a genotype and getting the mean(treatment x) / mean(treatment y) per day per trait.
b <- traits[traits$genotype == "PI_330182", ]
res <- lapply(sort(unique(b$day)), function(z) {d <- b[b$day == z, ]; res <- mapply(function(x, y) { res <- (mean(x) / mean(y)) ; return(res) },split(d[, names.numeric.cols], d$treatment)[["30"]], split(d[, names.numeric.cols], d$treatment)[["100"]])} )
names(res) <- sort(unique(b$day))
