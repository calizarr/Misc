...
rem.geno <- c("P0636AMX", "P1690AM")
b <- traits[traits$genotype %ni% rem.geno, ]
b$genotype <- as.factor(as.character(b$genotype))

all.treat <- lapply(split(b, b$treatment), function(x) {adj <- "fdr"; Dunnet.sum <- summary(glht(aov(area ~ genotype, data = x), linfct = mcp(genotype="Dunnett")), test = adjusted(adj))})

opar <- par()
sapply(names(all.treat),
  function(x) {
    png(filename = paste0(paste("treatment", x, "Dunnett", "test", sep = "_"), ".png"), width = 7*2, height = 7, res = 150, units = "in", pointsize = 12)
    par(mar = c(5, 8, 4, 5))
    plot(all.treat[[x]])
    title(main = paste0("Dunnett's Test: Many-to-One Comparisons, adjusted by: ", adj, " of treatment: ", x), line = 1, cex.main = 1)
    dev.off()
  }
)
par(opar)

## Dunnet <- glht(aov(area ~ genotype, data = b), linfct = mcp(genotype="Dunnett"), adjusted = "fdr")
## adj <- "fdr"
## Dunnet.sum <- summary(Dunnet, test = adjusted(adj))

## opar <- par()
## par(mar = c(5, 8, 4, 5))
## plot(Dunnet.sum)
## title(main = paste0("Dunnet's Test: Many-to-One Comparisons, adjusted by ", adj), line = 1, cex.main = 1)
## par(opar)
