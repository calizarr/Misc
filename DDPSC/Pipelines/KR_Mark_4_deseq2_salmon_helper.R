makeResultsTables <- function(treatments = treatments, dds = dds, threads = 4, gene.symbols = gene.symbols) {
    ...
    tables <- lapply(seq_along(treatments),
                     function(x) {
                         tabs <- lapply(seq_along(treatments),
                                function(y) {
                                    if (y <= x) {
                                        NULL
                                    } else {
                                        ...
                                        print(paste0("Summary_", treatments[x], "_vs_", treatments[y]))
                                        print(summary(resOrdered))
                                        sink()
                                        resOrderedDF <- as.data.frame(resOrdered)
                                        resOrderedDF$genes <- rownames(resOrderedDF)
                                        resOrderedDF[, paste0("baseMean.", treatments[x])] <- baseMeanPerSample[, treatments[x]]
                                        resOrderedDF[, paste0("baseMean.", treatments[y])] <- baseMeanPerSample[, treatments[y]]
                                        resOrderedDF <- resOrderedDF[, c(paste0("baseMean.", treatments[x]), setdiff(names(resOrderedDF), paste0("baseMean.", treatments[x])))]
                                        resOrderedDF <- resOrderedDF[, c(paste0("baseMean.", treatments[y]), setdiff(names(resOrderedDF), paste0("baseMean.", treatments[y])))]
                         ...
                         names(tabs) <- treatments
                         return(tabs)
                     }
                     )
    names(tables) <- treatments
