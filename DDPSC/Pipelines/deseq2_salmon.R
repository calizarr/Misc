...
Bd21.txi <- subset.txi("Bd21-0", txi)
...
rownames(sampleTable) <- merged.dap$fileNames
sampleTable$timepoint <- factor(sampleTable$timepoint, levels = sort(unique(merged.dap$DAP)))
sampleTable$group <- paste0(sampleTable$genotype,".",sampleTable$treatment)

## Splitting up the TimeCourse
vals.early <- paste0("-",paste(sort(unique(merged.dap$DAP))[2:3], collapse = "-|-"),"-")
vals.middle <- paste0("-",paste(sort(unique(merged.dap$DAP))[4:7], collapse = "-|-"),"-")
vals.late <- paste0("-",paste(sort(unique(merged.dap$DAP))[8:10], collapse = "-|-"),"-")
find.early <- grep(vals.early, names(files))
find.middle <- grep(vals.middle, names(files))
find.late <- grep(vals.late, names(files))

early.txi <- subset.txi(vals.early, txi)
middle.txi <- subset.txi(vals.middle, txi)
late.txi <- subset.txi(vals.late, txi)

early.sampleTable <- s2c[s2c$timepoint <= 25 & s2c$timepoint > 21, ]
early.sampleTable[, c("label", "genotype", "treatment", "replicate")] <- lapply(early.sampleTable[, c("label", "genotype", "treatment", "replicate")], factor)
early.sampleTable$group <- relevel(factor(paste0(early.sampleTable$genotype,".",early.sampleTable$treatment)), "Bd21-0.90%")
## early.sampleTable$timepoint <- factor(early.sampleTable$timepoint, levels = sort(unique(merged.dap$DAP))[2:3])
early.sampleTable$genotype <- relevel(early.sampleTable$genotype, "Bd21-0")
early.sampleTable$treatment <- relevel(early.sampleTable$treatment, "90%")
early.sampleTable$timepoint <- factor(early.sampleTable$timepoint)
rownames(early.sampleTable) <- paste0(early.sampleTable$genotype,"-", early.sampleTable$treatment, "-", early.sampleTable$timepoint, "-", early.sampleTable$replicate)
early.sampleTable <- early.sampleTable[colnames(early.txi$counts), ]

## early.ddsTC <- DESeqDataSetFromTximport(txi = early.txi, colData = early.sampleTable, design = ~ 0 + genotype*treatment*timepoint)
early.ddsTC <- DESeqDataSetFromTximport(txi = early.txi, colData = early.sampleTable, design = ~ 0 + group + timepoint + group:timepoint)

middle.txi <- subset.txi(paste0("Bd3-1-22.50%-|Bd21-0-22.50%-|Bd1-1-22.50%-"), middle.txi, inv = TRUE)

middle.sampleTable <- s2c[s2c$timepoint > 25 & s2c$timepoint < 39, ]
## middle.sampleTable <- middle.sampleTable[!(middle.sampleTable$genotype %in% c("Bd1-1", "Bd21-0", "Bd3-1") & middle.sampleTable$treatment %in% c("22.50%")),]
middle.sampleTable[, c("label", "genotype", "treatment", "replicate")] <- lapply(middle.sampleTable[, c("label", "genotype", "treatment", "replicate")], factor)
middle.sampleTable$group <- relevel(factor(paste0(middle.sampleTable$genotype,".",middle.sampleTable$treatment)), "Bd21-0.90%")
## middle.sampleTable$timepoint <- factor(middle.sampleTable$timepoint, levels = sort(unique(merged.dap$DAP))[4:7])
middle.sampleTable$genotype <- relevel(middle.sampleTable$genotype, "Bd21-0")
middle.sampleTable$treatment <- relevel(middle.sampleTable$treatment, "90%")
middle.sampleTable$timepoint <- factor(middle.sampleTable$timepoint)
rownames(middle.sampleTable) <- paste0(middle.sampleTable$genotype,"-", middle.sampleTable$treatment, "-", middle.sampleTable$timepoint, "-", middle.sampleTable$replicate)
middle.sampleTable <- middle.sampleTable[grep("Bd1-1-22.50%-|Bd21-0-22.50%-|Bd3-1-22.50%-", rownames(middle.sampleTable), invert = T), ]
middle.sampleTable <- middle.sampleTable[colnames(middle.txi$counts), ]

## middle.ddsTC <- DESeqDataSetFromTximport(txi = middle.txi, colData = middle.sampleTable, design = ~ 0 + genotype*treatment*timepoint)
middle.ddsTC <- DESeqDataSetFromTximport(txi = middle.txi, colData = middle.sampleTable, design = ~ 0 + group + timepoint + group:timepoint)

late.txi <- subset.txi("Bd1-1.45%-|Bd1-1.90%-|Bd1-1.Drought-", late.txi, inv = TRUE)
late.txi <- subset.txi("Bd1-1.22.50%-|Bd21-0.22.50%-|Bd3-1.22.50%-", late.txi, inv = TRUE)

late.sampleTable <- s2c[s2c$timepoint >= 39, ]
rownames(late.sampleTable) <- paste0(late.sampleTable$genotype,"-", late.sampleTable$treatment, "-", late.sampleTable$timepoint, "-", late.sampleTable$replicate)
late.sampleTable <- late.sampleTable[!(late.sampleTable$genotype %in% c("Bd1-1", "Bd21-0", "Bd3-1") & late.sampleTable$treatment %in% c("22.50%")),]
late.sampleTable <- late.sampleTable[!(grepl("Bd1-1.45%-|Bd1-1.90%-|Bd1-1.Drought-", rownames(late.sampleTable))), ]
late.sampleTable[, c("label", "genotype", "treatment", "replicate")] <- lapply(late.sampleTable[, c("label", "genotype", "treatment", "replicate")], factor)
late.sampleTable$group <- relevel(factor(paste0(late.sampleTable$genotype,".",late.sampleTable$treatment)), "Bd21-0.90%")
## late.sampleTable$timepoint <- factor(late.sampleTable$timepoint, levels = sort(unique(merged.dap$DAP))[8:10])
late.sampleTable$genotype <- relevel(late.sampleTable$genotype, "Bd21-0")
late.sampleTable$treatment <- relevel(late.sampleTable$treatment, "90%")
late.sampleTable$timepoint <- factor(late.sampleTable$timepoint)
late.sampleTable <- late.sampleTable[colnames(late.txi$counts), ]

## late.ddsTC <- DESeqDataSetFromTximport(txi = late.txi, colData = late.sampleTable, design = ~ 0 + genotype*treatment*timepoint)
late.ddsTC <- DESeqDataSetFromTximport(txi = late.txi, colData = late.sampleTable, design = ~ 0 + group + timepoint + group:timepoint)

samps <- sampleTable[sampleTable$genotype == "Bd21-0", ]
...
###### TIME SERIES VERSION ######
## formula.des <- "~ 0 + genotype + treatment + timepoint + genotype:treatment + treatment:timepoint + genotype:timepoint"
## formula.des <- "~ 0 + genotype*treatment*timepoint"
formula.des <- "~ 0 + group + timepoint + group:timepoint"

m1 <- model.matrix(as.formula(formula.des), sampleTable)
m1.mod <- apply(m1, 2, function(x) { all(x == 0) })
m1.final <- m1[, -m1.mod[which(m1.mod == TRUE)]]
attr(m1.final, "assign") <- attr(m1, "assign")[-unname(which(m1.mod == TRUE))]
attr(m1.final, "contrasts") <- attr(m1, "contrasts")


## (ddsTC <- DESeqDataSetFromTximport(txi = txi, colData = sampleTable, design = as.formula(formula.des)))
## (ddsTC <- DESeqDataSetFromTximport(txi = txi, colData = sampleTable, design = m1.final))
...
lapply(names(ddsTC), function(seg.time) {
    png(filename = paste0(seg.time, "_rlog_comparison_two_samples.png"), width = 480*2, height = 480*2, res = 100)
    par( mfrow = c( 1, 3 ) )
    ## lims <- c(-2, 20)
    lims <- c()
...
lapply(names(rldTC), function(seg.time) {
    ...
    pheatmap(sampleDistMatrix,
             ...
             col=colors,
             fontsize_row = 7,
             filename = paste0(seg.time, "_rlog_standard_heatmap.png"))
    ## dev.off()
})
...
lapply(names(vsdTC), function(seg.time) {
    ...
    pheatmap(sampleDistMatrix,
             ...
             col=colors,
             fontsize_row = 7,
             filename = paste0(seg.time, "_vsd_standard_heatmap.png")
             )
    ## dev.off()
    })
...
lapply(names(ddsTC), function(seg.time) {
    ...
    pheatmap(samplePoisDistMatrix,
             ...
             col=colors,
             fontsize_row = 7,
             filename = paste0(seg.time, "_", "rlog_poisson_heatmap.png")
             )
    ## dev.off()
})
...
lapply(names(ddsTC), function(seg.time) {
    ...
    pheatmap(samplePoisDistMatrix,
             ...
             col=colors,
             fontsize_row = 7,
             filename = paste0(seg.time, "_", "vsd_poisson_heatmap.png")
             )
    ## dev.off()
})
...
lapply(names(rldTC), function(seg.time) {
    ...
    ## (data <- plotPCA(rld, intgroup = c("group", "timepoint", "replicate"), returnData = TRUE))
        (data <- plotPCA(rld, intgroup = c("group", "timepoint"), returnData = TRUE))   
    ...
    p <- ggplot(data, aes(PC1, PC2)) + 
        geom_point(aes(color=group, shape = timepoint), size = 3) +
        xlab(paste0("PC1: ",percentVar[1],"% variance")) +
        ylab(paste0("PC2: ",percentVar[2],"% variance")) +
        coord_fixed()
    ## p <- plotPCA(rld, intgroup = c("group", "timepoint"))
    ggsave(paste0(seg.time,"_rlog_timecourse_PCA.png"), plot = p)
})
...
lapply(names(vsdTC), function(seg.time) {
    ...
    p <- ggplot(data, aes(PC1, PC2)) + 
        ...
        ylab(paste0("PC2: ",percentVar[2],"% variance")) +
        coord_fixed()
    ggsave(paste0(seg.time,"_vsd_timecourse_PCA.png"), plot = p)
})
...
lapply(names(rldTC), function(seg.time) {
    ...
    p <- fviz_pca_ind(pca, label = "all", invisible="ind", alpha.var = "contrib",
                      habillage = rld$group, addEllipses = T, jitter = list(what = "b", width = 10, height = 15))
    ggsave(paste0(seg.time, "_rlog_timecourse_PCA_treatment_fviz_pca_ind.png"))
})
...
lapply(names(vsdTC), function(seg.time) {
    ...
    p <- fviz_pca_ind(pca, label = "all", invisible="ind", alpha.var = "contrib",
                      habillage = rld$group, addEllipses = T, jitter = list(what = "b", width = 10, height = 15))
    ggsave(paste0(seg.time, "_vsd_timecourse_PCA_treatment_fviz_pca_ind.png"))
})
...
lapply(names(rldTC), function(seg.time) {
    ...
    ggplot(mds, aes(X1,X2,color=group,shape=timepoint)) + geom_point(size=3) +
        coord_fixed()
    ggsave(paste0(seg.time, "_rlog_timecourse_MDS.png"))
})
...
lapply(names(vsdTC), function(seg.time) {
    ...
    ggplot(mds, aes(X1,X2,color=group,shape=timepoint)) + geom_point(size=3) +
        coord_fixed()
    ggsave(paste0(seg.time, "_vsd_timecourse_MDS.png"))
})
...
lapply(names(ddsTC), function(seg.time) {
    ...
    ggplot(mdsPois, aes(X1,X2,color=group,shape=timepoint)) + geom_point(size=3) +
        coord_fixed()
    ggsave(paste0(seg.time, "_rlog_timecourse_Poisson_MDS.png"))
})
...
lapply(names(ddsTC), function(seg.time) {
    ...
    ggplot(mdsPois, aes(X1,X2,color=group,shape=timepoint)) + geom_point(size=3) +
        coord_fixed()
    ggsave(paste0(seg.time, "_vsd_timecourse_Poisson_MDS.png"))
})
...
ddsTC.run <- lapply(names(ddsTC), function(seg.time) {
    ...
    DESeq(ddsTC[[seg.time]], test = "LRT", reduced = ~ 0 + group + timepoint, parallel = TRUE)
})
names(ddsTC.run) <- c("early", "middle", "late")
...
names(resTC) <- c("early", "middle", "late")
...
lapply(names(resTC), function(seg.time) {
    ...
    ## dir.create(dired)
    resTC <- resTC[[seg.time]]
    ddsTC <- ddsTC.run[[seg.time]]
    resu <- names(resTC)[[1]]
    res <- resTC[[resu]]
    topGene <- rownames(res)[which.min(res$padj)]
    geneCounts <- plotCounts(ddsTC, gene=topGene, intgroup = c("group", "timepoint"), returnData = TRUE)
    p <- ggplot(geneCounts, aes(x = group, y = count, color = timepoint)) +
        scale_y_log10() +
        geom_point(position=position_jitter(width=.1, height=0), size=3) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
    ## ggsave(filename = file.path(dired, paste0(seg.time,"_",resu,"_",topGene,"_plotCounts.png")), plot = p)
    ggsave(filename = paste0(seg.time,"_",topGene,"_plotCounts.png"), plot = p)
    p <- ggplot(geneCounts, aes(x = group, y = count, fill = timepoint)) +
        scale_y_log10() +
        geom_dotplot(binaxis = "y", stackdir = "center") +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
    ggsave(filename = paste0(seg.time,"_", topGene, "_outline_plotCounts.png"))
    p <- ggplot(geneCounts, aes(x = group, y = count, color = timepoint, group = timepoint)) +
        scale_y_log10() + geom_point(size = 3) + geom_line() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
    ggsave(filename = paste0(seg.time,"_", topGene, "_structural_plotCounts.png"))
})
...
lapply(names(resTC), function(seg.time) {
    resTC <- resTC[[seg.time]]
    dired <- file.path(paste0("Time_", seg.time), "MAplots")
    dir.create(dired)
    lapply(names(resTC), function(rName) {
        png(filename = file.path(dired, paste0(seg.time, "_", rName,"_MAplot.png")), width = 480*2, height = 480*2, res = 100)
        res <- resTC[[rName]]
        plotMA(res, ylim = c(-5, 5))
        dev.off()
        png(filename = file.path(dired, paste0(seg.time, "_", rName,"_most.sig_MAplot.png")), width = 480*2, height = 480*2, res = 100)
        res <- resTC[[rName]]
        plotMA(res, ylim = c(-5, 5))
        topGene <- rownames(res)[which.min(res$padj)]
        with(res[topGene, ], {
...
lapply(names(resTC), function(seg.time) {
    resTC <- resTC[[seg.time]]
    dired <- file.path(paste0("Time_", seg.time), "Histograms")
    dir.create(dired)
    lapply(names(resTC), function(rName) {
        res <- resTC[[rName]]
        png(filename = file.path(dired, paste0(seg.time, "_", rName,"_Histogram.png")), width = 480*2, height = 480*2, res = 100)
...
lapply(names(ddsTC.run), function(seg.time) {
    ...
    data <- plotCounts(dds, which.min(res[[1]]$padj),
                       intgroup = c("timepoint", "group"), returnData = TRUE)
    data$group <- relevel(factor(gsub("(Bd\\d+-\\d)\\.(.*)", "\\1\n\\2",as.character(data$group))), "Bd21-0\n90%")
    p <- ggplot(data, aes(x = timepoint, y = log10(count), color = group, group = group)) +
        geom_point() + stat_smooth(se = FALSE, method = "loess") +
        theme(strip.text.x = element_text(size = 7.5)) +
        ## ggtitle(paste0("Condition-specific changes over time for: ", gene)) +
        ## scale_x_discrete(name = "Days after planting") +
        labs(y = "DESeq2 normalized counts", x = "Days after planting",
             title = paste0("Condition-specific changes over time for: ", gene)) +
        ## scale_y_log10(name = "DESeq2 normalized counts", breaks = c(500, 1000, 2000, 3000)) +
        ## coord_trans(y = "log10") +
        facet_grid(~ group)
    ggsave(filename = paste0(seg.time, "_", gene, "_timecourse_plotCounts.png"), plot = p, width = 10, height = 10)
    gene <- "Bradi4g42980"
    data <- plotCounts(dds, grep(gene, rownames(res[[1]])),
                       intgroup = c("timepoint", "group"), returnData = TRUE)
    data$group <- relevel(factor(gsub("(Bd\\d+-\\d)\\.(.*)", "\\1\n\\2",as.character(data$group))), "Bd21-0\n90%")
    p <- ggplot(data, aes(x = timepoint, y = log10(count), color = group, group = group)) +
        geom_point() + stat_smooth(se = FALSE, method = "loess") +
        ...
        labs(y = "DESeq2 normalized counts", x = "Days after planting",
             title = paste0("Condition-specific changes over time for: ", gene)) +
        ## scale_x_discrete(name = "Days after planting") +
        ## scale_y_log10(name = "DESeq2 normalized counts", breaks = c(min(data$count), max(data$count))) +
        ## coord_trans(y = "log10") +
        facet_grid(~ group)
    ggsave(filename = paste0(seg.time, "_", gene, "_timecourse_plotCounts_all.png"), plot = p, width = 10, height = 10)
})
...
lapply(names(resTC), function(seg.time) {
    res <- resTC[[seg.time]]
    dds <- ddsTC.run[[seg.time]]
    dired <- file.path(paste0("Time_", seg.time), "clusterHeatmaps")
    dir.create(dired)
    lapply(names(res), function(rName) {
        ...
        p <- pheatmap(mat, breaks=seq(from=-thr, to=thr, length=101), cluster_col = FALSE,
                      filename = file.path(dired, paste0(seg.time,"_",rName,"_clusterHeatmap.png")),
                      fontsize_col = 6)
    })
})

## Finding all significant genes, then joining with table and getting GO terms.
go.terms <- read.csv(file = file.path(annotation.path, go.file), sep = "\t")
...
genes <- lapply(names(resTC), function(seg.time) {
    ...
    ## tmp <- data.frame(unname( t(sapply(terms, function(x) {
    ##     toTable(GOTERM[x])[1,2:4]
    ## }))))
...
names(genes) <- c("early", "middle", "late")
