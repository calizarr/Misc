...
sampleTable$group <- factor(paste0(sampleTable$genotype,".",sampleTable$treatment))

## Splitting up the txi and sampleTable into genotypes
genos <- list(Bd21 = "Bd21-0", Bd3 = "Bd3-1", Bd1= "Bd1-1")
## find.genos <- lapply(genos, grep, names(files))
genos.txi <- lapply(genos, function(x) { subset.txi(x, txi) })
## control <- list(Bd21 = "Bd21-0-90%", Bd3 = "Bd3-1-90%", Bd1 = "Bd1-1-90%")
## control.txi <- lapply(control, function(x) { subset.txi(x, txi) } )
treats <- sort(unique(merged.dap$Treatment))[-3]

txi.sub <- lapply(genos, function(geno) {
    res <- lapply(treats, function(treat) {
        times.treat <- unique(sort(gsub("-\\d","", gsub(paste0(geno,"-",treat,"-"), "",grep(paste0(geno,"-",treat), colnames(txi$counts), val = T)))))
        times.control <- unique(sort(gsub("-\\d","", gsub(paste0(geno,"-","90%","-"), "",grep(paste0(geno,"-","90%"), rownames(sampleTable), val = T)))))
        times <- intersect(times.control, times.treat)
        treat.search <- paste(unname(sapply(times, function(x) { paste0(geno, "-", treat, "-", x)})), collapse = "|")
        control.search <- paste(unname(sapply(times, function(x) { paste0(geno, "-", "90%", "-",x)})), collapse = "|")
        if(treat.search == "") {
            return(NULL)
        } else {
            sub <- subset.txi(paste0(treat.search,"|", control.search), txi)
        }
    })
    names(res) <- treats
    return(res)
...
## All things have been subsetted properly except sampleTable
sampleTable.sub <- lapply(genos, function(geno) {
    res <- lapply(treats, function(treat) {
        times.treat <- unique(sort(gsub("-\\d","", gsub(paste0(geno,"-",treat,"-"), "",grep(paste0(geno,"-",treat), rownames(sampleTable), val = T)))))
        times.control <- unique(sort(gsub("-\\d","", gsub(paste0(geno,"-","90%","-"), "",grep(paste0(geno,"-","90%"), rownames(sampleTable), val = T)))))
        times <- intersect(times.control, times.treat)
        treat.search <- paste(unname(sapply(times, function(x) { paste0(geno, "-", treat, "-", x)})), collapse = "|")
        control.search <- paste(unname(sapply(times, function(x) { paste0(geno, "-", "90%", "-",x)})), collapse = "|")
        ...
        sampTable <- sampleTable[grep(paste0(treat.search,"|",control.search), rownames(sampleTable), invert = F), ]
        sampTable <- droplevels(sampTable)
        sampTable$group <- relevel(sampTable$group, paste0(geno,".","90%"))
        sampTable$treatment <- relevel(sampTable$treatment, "90%")
        time.ref <- sort(unique(times))[1]
        sampTable$timepoint <- relevel(sampTable$timepoint, sort(unique(times))[1])
        sampTable <- sampTable[colnames(txi.sub[[gsub("-\\d", "", geno)]][[treat]]$counts), ]
    })
    names(res) <- treats
    return(res)
})

txi.sub <- unlist(txi.sub, recursive = F)
## Removing null list objects
txi.sub <- txi.sub[!(unname(sapply(txi.sub, is.null)))]
sampleTable.sub <- unlist(sampleTable.sub, recursive = F)
## Removing null list objects
sampleTable.sub <- sampleTable.sub[!(unname(sapply(sampleTable.sub, is.null)))]

ddsTC <- lapply(names(txi.sub), function(group) {
    print(group)
    DESeqDataSetFromTximport(txi = txi.sub[[group]], colData = sampleTable.sub[[group]], design = ~ 0 + treatment + timepoint + treatment:timepoint)
})
