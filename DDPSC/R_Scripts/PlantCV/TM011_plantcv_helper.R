...
trait.heatmap <- function(dfr, diff1, diff2, title, measure,
  ...
  if(save) {
    dev.off()
    if(big) {
      png(file = paste0("pdf_images/aggregate/", save.name, ".png"), width = 7*6, height = 7*3, res = 150, pointsize = 8, units = "in")
    } else {
      png(file = paste0("pdf_images/aggregate/", save.name, ".png"), width = 7*2, height = 7, res = 150, pointsize = 8, units = "in")
    }
    eval(reorg$call)
    dev.off()
  }
