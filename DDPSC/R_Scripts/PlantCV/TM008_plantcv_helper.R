...
################################# BEGIN: Water Use Efficiency Function ####################################
...
################################# END: Water Use Efficiency Function #################################
...
################ Comparing one genotype against many others each treatment separately ################
oneToManyComps <- function(dfr, genotypes, trait, geno, adj.meth = "fdr", rem.geno = c("B73", "P0636AMX", "P1690AM"), geno.col = "genotype", treat.col = "treatment") {
  tests <- lapply(split(dfr, dfr[, treat.col]),
    function(x) {
      sapply(genotypes,
        function(y) {
          x.trait <- x[x[, geno.col] == y, trait]
          y.trait <- x[x[, geno.col] == geno, trait]
          if ( y %in% rem.geno || length(rm.na(x.trait)) <= 1) {
            none <- list()
            none$p.value <- NA
            return(none)
          } else {
            res <- t.test(x.trait, y.trait)
            return(res)
          }
        }
      )
  ...
  tests.pvals <- lapply(tests, function(x) { lapply(x, function(y) { y$p.value }) })
  tests.pvals <- do.call(rbind, lapply(tests.pvals, data.frame))
  tests.pvals <- tests.pvals[, colnames(tests.pvals) %ni% rem.geno]
  tests.pvals <- data.frame(t(tests.pvals))

  pvals.adj <- data.frame(sapply(tests.pvals, p.adjust, method = adj.meth, n = nrow(tests.pvals)*ncol(tests.pvals)))
  rownames(pvals.adj) <- rownames(tests.pvals)

  kd <- which(pvals.adj < 0.05, TRUE)
  kd <- cbind(kd, as.integer(gsub("X", "", apply(kd, 1, function(x) { colnames(pvals.adj)[x[2]] }))))
  kd <- cbind(kd, apply(kd, 1, function(x) eval(parse(text = paste0("pvals.adj[", paste(x[1:2], collapse = ","),"]")))))
  colnames(kd) <- c(colnames(kd)[1:2], "treatment", "p.value")
  return(list(Orig.Tests = tests, Pvals = tests.pvals, Signif = kd))
}
