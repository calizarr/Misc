...
project.code <- "TM007_E_082415"
source("TM007_plantcv_data_stub.R")
...
##   p <- plot.trait(traits, splits[[index]], treatments, "dap", trait, paste0("(",units,")"), limits = c(-5, 270), facet = TRUE, x.text.angle = 90, breaks = breaks, labels = lables, x.scale = "Days After Planting", print.plot = FALSE)
...
for(genotype in genotypes) {
  p <- plot.trait(wue.data, genotype, treatments, "day", "WUE", units, limits = c(), y.scale = "Water Use Efficiency", x.scale = "Days After Planting")
  p <- p + scale_colour_discrete(name = "Treatment", breaks = breaks, labels = labels, limits = levels(as.factor(traits$treatment)))
  print(p)
  ggsave(paste0("images/",paste(genotype,"wue", units.save, sep="_"),".png"),  dpi = 100, width = 7, height = 7, units="in")
}
dev.off()

## Facet plot of the Water Use Efficiency
trait <- "WUE"
print.units <- "pixels_g"
...
splits <- split(genotypes, ceiling(seq_along(genotypes)/8))
for( index in seq_along(splits) ) {
  p <- plot.trait(wue.data, splits[[index]], treatments, "dap", "WUE", "(pixels / g)", limits = c(), facet = TRUE, x.text.angle = 90, breaks = breaks, labels = labels, x.scale = "Days After Planting")
  ggsave(paste0("pdf_images/aggregate/",paste("all","WUE", "geno", index, "pixels_g","facet",sep="_"),".png"), dpi = 300, width = 14, height = 7, units = "in")
}
...
for(genotype in genotypes) {
  plot.trait(rgr.sub, genotype, treatments, "day", "RGR.area", "(pixels/day)", limits = c(), breaks = breaks, labels = labels, x.scale = "Days After Planting", print.plot = TRUE)
  ggsave(paste0("images/",paste(genotype,"RGR.area","(pixels_day)",sep="_"),".png"),  dpi = 100, width = 7, height = 7, units="in" )
}
dev.off()

trait <- "RGR.area"
units <- "(pixels/day)"
print.units <- "pixels_day"
## Facet plot of all genotype traits.
...
splits <- split(genotypes, ceiling(seq_along(genotypes)/8))
for( index in seq_along(splits) ) {
  p <- plot.trait(rgr.sub, splits[[index]], treatments, "day", "RGR.area", "(pixels/day)", limits = c(), facet = TRUE, x.text.angle = 90, breaks = breaks, labels = labels, x.scale = "Days After Planting")
  ggsave(paste0("pdf_images/aggregate/",paste("all","RGR.area", "geno", index, "pixels_day","facet",sep="_"),".png"), dpi = 300, width = 14, height = 7, units = "in")
}
...
limits <- list(x.min = c(), x.max =c(), y.min = c(), y.max = c())
for(day in sort(unique(traits.avg$day))) {
  ...
  x.ranges <- ggplot_build(p)$panel$ranges[[1]]$x.range
  y.ranges <- ggplot_build(p)$panel$ranges[[1]]$y.range
  limits[["x.min"]] <- c(x.ranges[[1]], limits[["x.min"]])
  limits[["x.max"]] <- c(x.ranges[[2]], limits[["x.max"]])
  limits[["y.min"]] <- c(y.ranges[[1]], limits[["y.min"]])
  limits[["y.max"]] <- c(y.ranges[[2]], limits[["y.max"]])
}

xlim <- c(min(limits$x.min), max(limits$x.max))
ylim <- c(min(limits$y.min), max(limits$y.max))
for(day in sort(unique(traits.avg$day))) {
  p <- ggplot(traits.avg[traits.avg$day == day, ], aes(x = area, fill = as.factor(treatment))) + geom_density(alpha=0.3)
...
## Full data frame including replicates
merge.traits.wue <- merge(traits, wue.data, all = TRUE, by = c(names.non.numeric, "dap", "area"))
final.merge <- merge.traits.wue
final.merge.order <- final.merge[with(final.merge, order(group, day)), ]
final.non.num <- names(final.merge.order)[names(final.merge.order) %ni% grep("area|height|WUE|water", names(final.merge.order), value = T)]
final.merge.order <- final.merge.order[, c(final.non.num, setdiff(names(final.merge.order), final.non.num))]

final.merge.name <- paste0("all_traits_", project.code, ".csv")
write.csv(final.merge.order, final.merge.name, row.names = FALSE)
...
## GTT = genotype treatment timeGarde
